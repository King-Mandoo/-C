...
{						{
  ...
  A. 그건 바로 scanf() 와 gets()의 차이점에 있다. 
     scanf()는 표준입력의 버퍼에서 공백문자(스페이스, 탭, '\n' 등)를 처음 만나도 이를 배열에 저장하지 않고
     문자부터 다시 공백문자를 만날 때까지의 문자들을 배열에 저장한다.
	  
     반면에 gets() 함수는 표준입력의 버퍼의 맨 처음으로 만나는 '\n'까지의 모든 것을 배열에 저장한다.
	  
     따라서 scanf() 함수를 사용한 직후에 gets() 함수를 사용하게 되면, gets() 함수는 scanf() 함수가 남겨둔
     '\n'을 읽게되고, 종료가 되어버린다.
	  
     하지만 gets() 함수를 사용한 직후에 scanf() 함수를 사용한다면, 버퍼에 맨 처음에 남겨져있는 '\n'을 읽지
     않고 입력을 받게 된다.
	  
     "Hello"를 입력, 그리고 버퍼에 Hello \n 저장
     gets() 함수로 입력을 할 땐 이미 버퍼에 값이 저장 
